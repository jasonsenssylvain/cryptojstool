...
const ECKey           = require('../src/eckey.js');

describe('ECKey', function () {
  describe('+ ECKey()', function () {
    it('should create a new ECKey', function () {
      let buf = secureRandom(32, {type: 'Buffer'});
      let key = new ECKey(buf);
      assert.equal(key.toString(), buf.toString('hex'));
  ...
  describe('> when input is an Uint8Array', function () {
    it('should create a new ECKey ', function () {
      ...
      assert.equal(key.toString(), new Buffer(bytes).toString('hex'))
      assert.equal(key.compressed, true)
    })
  });
